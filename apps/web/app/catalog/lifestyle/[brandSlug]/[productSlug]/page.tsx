...
export default async function LifestyleProductPage({
  params,
}: {
  params: Promise<{ brandSlug: string; productSlug: string }>;
}) {
  const { brandSlug, productSlug } = await params;
  const product = await fetchLifestyleProductBySlug(brandSlug, productSlug);

  if (!product) notFound();

  const relatedVariants = (await fetchLifestyleRelatedProducts(brandSlug, product.sourceProductSlug)).filter(
  ...
  const displayProductTitle = getLifestyleDisplayProductTitle(product);
